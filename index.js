...
async function run() {
  try {
    ...
    const serviceCollection = client.db("carProvider").collection("services");
    const bookingCollection = client.db("carProvider").collection("bookings");

    // token or auth related api
...
app.get("/", (req, res) => {
  res.send("doctore is running pecefully");
});
